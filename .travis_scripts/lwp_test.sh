...
FILEOUT=output LD_LIBRARY_PATH=..:bin:$LD_LIBRARY_PATH LD_PRELOAD=ptrutil.so python ../kid_1D.py --backend=OpenMP
# test the results
cd output
ncdiff -O -v liquid_water_path 1D_out.nc ../../refdata/w3_N50_NORAIN.nc diff.nc
# check if the difference is small enough
ncap2 -A -s "diff_flag=liquid_water_path<1.5e-3 && liquid_water_path>-1.5e-3" diff.nc
...
# check if the difference is small enough
ncap2 -A -s "diff_flag=liquid_water_path<1.6e-3 && liquid_water_path>-1.6e-3" diff.nc
ncra -O diff.nc diff_mean.nc
ncks -V -v diff_flag -C -H diff_mean.nc
ncks -V -v diff_flag -C -H diff_mean.nc | grep '1'
cd ..
