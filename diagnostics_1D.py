...
def diagnostics(particles, arrays, it, size_x, size_z, first_timestep):
  ...
  for i in range(params["n_bins"]):
    ...
    particles.diag_wet_mom(0)
    arrays["mom_0"][i,:,:] = np.frombuffer(particles.outbuf()).reshape(size_x-2, size_z) * arrays["rhod_kid"]
    # computing 3-rd moment                          
    particles.diag_wet_mom(3)
    arrays["mom_3"][i,:,:] = np.frombuffer(particles.outbuf()).reshape(size_x-2, size_z) * arrays["rhod_kid"]
